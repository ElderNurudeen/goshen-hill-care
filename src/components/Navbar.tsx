...
export function Navbar() {
  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm py-2' : 'bg-white/80 backdrop-blur-sm py-3'}`}>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 group">
            <div className="bg-lilac-500 p-1.5 sm:p-2 rounded-lg text-white group-hover:bg-lilac-600 transition-colors">
              <HeartHandshake size={20} />
            </div>
            <div className="flex flex-col">
              <span className="font-bold text-charcoal text-sm sm:text-base leading-none">
                Goshen-hill
              </span>
              <span className="text-[10px] sm:text-xs text-lilac-600 font-medium tracking-wide">
                Care Limited
              </span>
            </div>
          </Link>
